...
class ReportUser extends Component { 
  ...
  async handleSubmit(event) {
    try {
      ...
      alert('A  user report submitted: ' + this.state.value) ;
      const docRef = await addDoc(collection(db, "reportedPost"), {
        message: this.state.value,
        userId: user.uid,
        ReportedUserId: this.props.user,
      ...
      <View style={styles.viewContainer}>
           ...
           <Card style={styles.viewContainer}>
                 <Headline style={styles.centerText}>
                   report potential violation of User policy
                 </Headline>
                 <Caption style={styles.centerText}>
                  report user with id {user}
                 ...
                 <TextInput
                     ...
                     <Card style={styles.viewContainer}>  
                     {this.state.image? ( <Card.Cover source={{ uri: this.state.image}} />) : null}
               ...
               <Button color='grey' style={styles.button} mode="contained" >send</Button>
               </TouchableOpacity>
...
export  function ReportUserWithProps(props){
  const navigation = useNavigation();
  const route = useRoute();
  const { itemId, report , user } = route.params;
 ...
 return(
  <ReportUser navigation={navigation} route={route}  user={user}/>
 )
}
